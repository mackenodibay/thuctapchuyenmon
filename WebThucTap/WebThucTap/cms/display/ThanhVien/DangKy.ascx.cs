...
public partial class cms_display_ThanhVien_DangKy : System.Web.UI.UserControl
{
    ...
    protected void lbtDangKy_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            string matkhau = Webbanhang.MaHoa.MaHoaMD5(tbMatKhau.Text);

            Webbanhang.KhachHang.Khachang_Inser(tbHoTen.Text, tbDiaChi.Text, tbSoDienThoai.Text, tbEmail.Text, matkhau, "");
    ...
    private bool DaTonTaiEmail(string email)
    {
        ...
        dt = Webbanhang.KhachHang.Thongtin_Khachhang_by_emailkh(email);
        if (dt.Rows.Count > 0)
            return true;
        else
            return false;
